...
/**
 * Description of kategori_siswa
 *
 * @author white
 */
class kategori_siswa extends CI_Controller {

    var $limit = 20;

    public function __construct() {
        parent::__construct();
        $this->load->model('m_kategori_siswa', 'the_m');
    ...
    function index() {
        ...
        $this->layout->set_title('kategori siswa');
        $this->layout->set_meta('Kategori siswa dari KPMAK UGM');
        ...
        $this->breadcrumb->add_crumb('kategori siswa');

        $data['primary_title'] = '<i class="ion-android-note"></i> Kategori';
        $data['sub_primary_title'] = 'siswa';
        $data['list'] = $this->the_m->get($this->limit, $offset)->result_array();
        $jumlah = $this->the_m->get("", "")->num_rows();
        $data['pagination'] = $this->_pagination($jumlah, $this->limit, "kategori_siswa/index", 3);
        $data['notif'] = $this->_notification();
        $this->layout->back('kategori_siswa/index', $data);
    }

    function add() {
        $this->rule->type('C');
        $this->layout->set_title('Tambah Kategori siswa');
        $this->breadcrumb->clear();
        $this->breadcrumb->add_crumb('Beranda', site_url());
        $this->breadcrumb->add_crumb('Kategori siswa', site_url('kategori_siswa'));
        $this->breadcrumb->add_crumb('Tambah Kategori siswa');
        $data['primary_title'] = 'Kategori siswa';
        $data['sub_primary_title'] = 'Proses tambah data';
        //menggunakan layout back/backend templating
        $this->layout->back('kategori_siswa/add', $data);
    }
    ...
    function save() {
        ...
        $this->form_validation->set_rules('kategori', 'kategori siswa', 'required');
        ...
        redirect('kategori_siswa');
    }

    function edit($id) {
        $this->rule->type('U');
        $data['list'] = $this->the_m->get_by($id);
        $this->layout->set_title('Edit Kategori siswa');
        $this->breadcrumb->clear();
        $this->breadcrumb->add_crumb('Beranda', site_url());
        $this->breadcrumb->add_crumb('Kategori siswa', site_url('kategori_siswa'));
        $this->breadcrumb->add_crumb('Edit Kategori siswa');
        //judul besar
        $data['primary_title'] = 'Kategori siswa';
        $data['sub_primary_title'] = 'Proses edit data';
        //mengaktifkan menu master di sidebar
        $data['arikel'] = 'active';
        $this->layout->back('kategori_siswa/edit', $data);
    }
