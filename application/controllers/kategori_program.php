...
/**
 * Description of kategori_program
 *
 * @author white
 */
class kategori_program extends CI_Controller {

    var $limit = 20;

    public function __construct() {
        parent::__construct();
        $this->load->model('m_kategori_program', 'the_m');
    ...
    function index() {
        ...
        $this->layout->set_title('Kategori Program');
        $this->layout->set_meta('Kategori Program dari AL-Qodar');
        ...
        $this->breadcrumb->add_crumb('Kategori Program');

        $data['primary_title'] = '<i class="ion-android-note"></i> Kategori';
        $data['sub_primary_title'] = 'Program';
        $data['list'] = $this->the_m->get($this->limit, $offset)->result_array();
        $jumlah = $this->the_m->get("", "")->num_rows();
        $data['pagination'] = $this->_pagination($jumlah, $this->limit, "kategori_program/index", 3);
        $data['notif'] = $this->_notification();
        $this->layout->back('kategori_program/index', $data);
    }
    ...
    function add() {
        ...
        $this->breadcrumb->clear();
        $this->breadcrumb->add_crumb('Beranda', site_url());
        $this->breadcrumb->add_crumb('Kategori Program', site_url('kategori_program'));
        $this->breadcrumb->add_crumb('Tambah Kategori Program');
        $data['primary_title'] = 'Kategori Program';
        $data['sub_primary_title'] = 'Proses tambah data';
        //menggunakan layout back/backend templating
        $this->layout->back('kategori_program/add', $data);
    }
    ...
    function save() {
        ...
        $this->form_validation->set_rules('kategori', 'Kategori Program', 'required');
        ...
        redirect('kategori_program');
    }

    function edit($id) {
        $this->rule->type('U');
        $data['list'] = $this->the_m->get_by($id);
        $this->layout->set_title('Edit Kategori Program');
        $this->breadcrumb->clear();
        $this->breadcrumb->add_crumb('Beranda', site_url());
        $this->breadcrumb->add_crumb('Kategori Program', site_url('kategori_program'));
        $this->breadcrumb->add_crumb('Edit Kategori Program');
        //judul besar
        $data['primary_title'] = 'Kategori Program';
        $data['sub_primary_title'] = 'Proses edit data';
        //mengaktifkan menu master di sidebar
        $data['arikel'] = 'active';
        $this->layout->back('kategori_program/edit', $data);
    }
